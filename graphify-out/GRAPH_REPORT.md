...
## Corpus Check
- 237 files · ~433,785 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1678 nodes · 3627 edges · 116 communities (95 shown, 21 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 20 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `08e434e7`
...
2. `Clip` - 43 edges
3. `ensureBuiltinCommands()` - 31 edges
4. `Overlay` - 31 edges
5. `isGapClip()` - 30 edges
6. `EditorApi` - 29 edges
7. `clipDur()` - 29 edges
8. `Chat()` - 28 edges
...
- `buildTimelineEnergyEvidence()` --indirect_call--> `clipEnabled()`  [INFERRED]
  web/lib/editor/semanticTimeline.ts → web/lib/editor/model.ts
...
Cohesion: 0.06
Nodes (49): maxDuration, POST(), runtime, SlashCmd, Conversation, CANCELLED_RESULT, isToolHistoryValid(), repairToolMessages() (+41 more)
...
### Community 13 - "commands.builtin.ts"
Cohesion: 0.18
Nodes (23): ensureBuiltinCommands(), addClip(), splitClip(), trimClip(), makeImageOverlay(), makeTextOverlay(), nextZ(), createVideoTrack() (+15 more)

### Community 14 - "cli.py"
Cohesion: 0.08
Nodes (40): ArgumentParser, build_parser(), Config, config_from_args(), _fmt(), main(), _print_summary(), KeepInterval (+32 more)
...
Cohesion: 0.32
Nodes (11): deleteClipRange(), deleteClipsAt(), intersectClipsWithSpeech(), keepSourceRange(), mergeOverlappingSameSource(), normalizeGeneratedCuts(), snapSpeechToWords(), c() (+3 more)

### Community 20 - "models.ts"
Cohesion: 0.12
Nodes (16): FINALS, getOpcodes(), lcsMatches(), normalizeHebrew(), Op, scriptKeepMask(), buildKeepIntervals(), DEFAULT_FILLERS (+8 more)

### Community 22 - "semanticTimeline.ts"
Cohesion: 0.16
Nodes (15): analyzeAudio(), avgDb(), cache, EnergyProfile, findSilences(), fp(), buildTimelineEnergyEvidence(), buildTimelineEvidence() (+7 more)

### Community 23 - "subtitlesEdl.ts"
Cohesion: 0.25
Nodes (16): assembledWords(), CaptionBuildOpts, CaptionMode, edlToCues(), edlToCuesWithScript(), edlToSrt(), endsPhrase(), phraseChars() (+8 more)
...
Cohesion: 0.25
Nodes (13): CaptionBg, CaptionPosition, captionStyleToCss(), DEFAULT_CAPTION_STYLE, normalizeCaptionStyle(), collapseProgressiveForBurn(), CaptionLayout, captionLayoutForTarget() (+5 more)
...
Cohesion: 0.05
Nodes (83): DashboardPage(), DialogState, fmtDate(), fmtRelativeHe(), ProjectCard(), userAvatarUrl(), userLabel(), COMMAND_ICONS (+75 more)
...
### Community 35 - "model.ts"
Cohesion: 0.14
Nodes (34): ClipInspector(), InspectorFocus, KIND, num(), OverlayInspector(), SubInspector(), SOURCE_COLORS, Timeline() (+26 more)
...
Cohesion: 0.11
Nodes (18): D-001 — עיבוד וידאו מקומי, לא בשרת, D-002 — ליבה משוכפלת web/local, D-003 — מפתחות בצד לקוח / env של המשתמש, D-004 — סוכן AI מאושר במסגרת v0.3.0, D-005 — Graphify כניווט ברירת מחדל לסוכנים, D-006 — Continuity דרך Git בלבד, D-007 — התנהגות סוכן אחרי בחירת סקריפט, D-008 — רצועות וידאו מרובות + סוכן דרך EditorApi (+10 more)
...
### Community 61 - "time.ts"
Cohesion: 0.14
Nodes (27): InspectorPanel(), titleFor(), TimelineToolbar(), clampTime(), clampZoom(), formatQuoteTime(), formatTimecode(), MS (+19 more)
...
Cohesion: 0.20
Nodes (16): GET(), runtime, GET(), configuredProviders(), classifyPublicKey(), decodeJwtPayload(), getAuthDiagnostics(), getRawPublicKey() (+8 more)
...
### Community 67 - "Clip"
Cohesion: 0.11
Nodes (24): ChatProps, Props, Props, Props, Props, EditorSnapshot, Updater, CanvasSize (+16 more)
...
Cohesion: 0.19
Nodes (17): migrateClips(), migrateState(), audioMuted(), audioTrack(), captionLocked(), captionTrack(), clampHeight(), DEFAULT_CANVAS (+9 more)

### Community 70 - "KeepInterval"
Cohesion: 0.42
Nodes (3): KeepInterval, קטע שנשמור בעריכה (על ציר הזמן המקורי)., HebrewCaptionGroupingTests
...
Cohesion: 0.20
Nodes (7): CORNERS, DragState, Handle, PreviewOverlays(), OverlayKind, overlayVisibleAt(), VisualTransform
...
Cohesion: 0.13
Nodes (26): mediaById(), uid(), extOf(), extractAssembledAudio(), extractAudio(), extractAudioChunks(), extractAudioSegment(), extractFrame() (+18 more)
...
Cohesion: 0.20
Nodes (16): buildCues(), buildSrt(), CaptionMode, Cue, endsPhrase(), endsSentence(), formatCueText(), formatTimestamp() (+8 more)
...
### Community 111 - "providers/policy.ts"
Cohesion: 0.36
...
### Community 115 - "assembleTranscript.ts"
Cohesion: 0.36
Nodes (7): assembledDuration(), AssembleOpts, assembleTranscript(), formatTranscriptLines(), WordsBySource, GAP_SOURCE, isSpeechWord()

## Knowledge Gaps
- **523 isolated node(s):** `Current task`, `Branch`, `Latest commit`, `Status`, `Exact continuation point` (+518 more)
...
- **Why does `Clip` connect `Clip` to `model.ts`, `commands.ts`, `Chat.tsx`, `project.ts`, `ffmpeg.ts`, `commands.builtin.ts`, `tools.ts`, `graph.integration.test.ts`, `assembleTranscript.ts`, `clipFilter.ts`, `models.ts`, `semanticTimeline.ts`, `subtitlesEdl.ts`, `app/page.tsx`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `EditorApi` connect `Clip` to `runtime.ts`, `commands.ts`, `Chat.tsx`, `commands.builtin.ts`, `tools.ts`, `app/page.tsx`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `MediaAsset` connect `Clip` to `thumbnails.ts`, `model.ts`, `commands.ts`, `Chat.tsx`, `overlay.ts`, `ffmpeg.ts`, `tools.ts`, `graph.integration.test.ts`, `app/page.tsx`?**
