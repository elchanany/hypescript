...
export function LoadingState({ label = "טוען נתונים…", compact = false }: { label?: string; compact?: boolean }) {
  return (
    <div className={`loading-state${compact ? " compact" : ""}`} role="status" aria-live="polite" aria-label={label}>
      <HypescriptBrandSpinner size={compact ? "xs" : "sm"} label={label} />
    </div>
  );
}
...
export function UploadProgressCard({ value, compact = false }: { value: TransferProgress; compact?: boolean }) {
  ...
  return (
    <section className={`upload-progress-card${compact ? " compact" : ""}`} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent} aria-label={`מעלה ${value.fileName}`}>
