...
export default function LandingDeviceShowcase() {
  ...
  useEffect(() => {
    const delay = phase === 0 ? 1200 : phase === 1 ? 1700 : 2600;
    const timer = window.setTimeout(() => {
      if (phase === 2) { setPrompt((value) => (value + 1) % prompts.length); setPhase(0); }
      else setPhase((value) => value + 1);
    }, delay);
    return () => window.clearTimeout(timer);
  }, [phase, prompt, prompts.length]);

  function choosePrompt(index: number) { setPrompt(index); setPhase(0); }
    ...
    <section className="hsx-device-story hsx-reveal" aria-label={copy.label}>
      <div className="hsx-device-copy">
        ...
        <div className="hsx-prompt-switcher" role="tablist" aria-label={copy.tabs}>
          {prompts.map((item, index) => <button key={item[0]} type="button" role="tab" aria-selected={prompt === index} onClick={() => choosePrompt(index)}>{index + 1}</button>)}
        </div>
      </div>
        ...
        <div className="hsx-tablet">
          ...
          <div className="hsx-tablet-ui">
            ...
            <div className="hsx-tablet-work">
              <div className="hsx-tablet-video" data-phase={phase}><img src={demoImage} alt="" /><span className="hsx-preview-brand"><BrandLogo variant="icon" size="xs" decorative /></span><span className="hsx-tablet-action"><WandSparkles size={11}/>{phase === 0 ? copy.working : phase === 1 ? copy.editing : copy.ready}</span><button type="button" aria-label={copy.play}><Play size={18} fill="currentColor" /></button><strong>{copy.caption}</strong></div>
              <aside key={`${prompt}-${phase}`}><span><Sparkles size={12} />Hype AI</span><p className="hsx-demo-user">{prompts[prompt][0]}</p>{phase === 0 ? <div className="hsx-demo-thinking" aria-label={copy.working}><i/><i/><i/></div> : <div className={phase === 2 ? "done" : ""}><Check size={12} />{prompts[prompt][1]}</div>}</aside>
            </div>
            <div className="hsx-tablet-timeline" data-phase={phase}><i /><i /><i /><i /><i /><b /></div>
          </div>
        </div>

        <div className="hsx-phone">
          <div className="hsx-phone-notch" />
          <header><span><BrandLogo variant="icon" size="xs" decorative /></span><div><b>Hype AI</b><small>{copy.connected}</small></div><i className={phase < 2 ? "busy" : ""} /></header>
          <div className="hsx-phone-preview" data-phase={phase}><img src={demoImage} alt="" /><i className="hsx-preview-brand"><BrandLogo variant="icon" size="xs" decorative /></i><span>00:35</span><b><WandSparkles size={10}/>{phase === 0 ? copy.working : phase === 1 ? copy.editing : copy.ready}</b></div>
          <div className="hsx-phone-chat" key={`${prompt}-${phase}`} aria-live="polite">
            <p className="user">{prompts[prompt][0]}</p>{phase === 0 ? <p className="agent typing"><i/><i/><i/></p> : <p className="agent"><WandSparkles size={12} />{prompts[prompt][1]}</p>}{phase === 2 && <div><Check size={12} />{copy.ready}</div>}
          </div>
          <footer><span>{copy.change}</span><button type="button" aria-label={copy.send} onClick={() => choosePrompt((prompt + 1) % prompts.length)}><Send size={12} /></button></footer>
        </div>
