...
import {
  ArrowLeft, Captions, Check, Cloud, Download, Film, Layers3,
  MessageSquareText, Play, Scissors, ShieldCheck, Sparkles, Wand2,
} from "lucide-react";
...
const plans = [
  { name: "Free", price: "₪0", text: "לנסות, ללמוד וליצור פרויקטים קצרים.", items: ["3 פרויקטים", "2GB אחסון", "10 דקות רינדור בחודש"] },
  { name: "Creator", price: "בקרוב", text: "ליוצרים, עורכים ועסקים שמפרסמים באופן קבוע.", items: ["50 פרויקטים", "20GB אחסון", "120 דקות רינדור בחודש"], featured: true },
  { name: "Pro", price: "בקרוב", text: "לצוותים ולנפח עבודה מקצועי.", items: ["500 פרויקטים", "100GB אחסון", "480 דקות רינדור בחודש"] },
];
  ...
  return (
    <main className="marketing-page">
      ...
      <section className="marketing-hero">
        <div className="marketing-kicker"><Sparkles size={15} />עריכת וידאו מקצועית, בלי להילחם בטיימליין</div>
      ...
      <section className="marketing-section marketing-how" id="how">
        ...
        <div className="marketing-steps">
          ...
          <article><b>02</b><Wand2 size={24} /><h3>עורכים</h3><p>חותכים בטקסט או מבקשים מעוזר העריכה לבצע סדרת פעולות.</p></article>
      ...
      <section className="marketing-section" id="pricing">
        ...
        <div className="marketing-pricing">
          {plans.map((plan) => <article key={plan.name} className={plan.featured ? "featured" : ""}>{plan.featured && <em>הכי מתאים ליוצרים</em>}<h3>{plan.name}</h3><div className="marketing-price">{plan.price}</div><p>{plan.text}</p><ul>{plan.items.map((item) => <li key={item}><Check size={15} />{item}</li>)}</ul><Link href="/login?next=/dashboard" className={`btn ${plan.featured ? "primary" : "secondary"}`}>התחל בחינם</Link></article>)}
        </div>
      </section>
