...
const INPUT_SCHEMAS: Record<CommandId, CommandSchema> = {
  ...
  "clip.setEnabled": schema(["id", "enabled"], { id, enabled: bool }), "clip.setVolume": schema(["id", "volume"], { id, volume: num }), "clip.setOpacity": schema(["id", "opacity"], { id, opacity: num }), "clip.setColorAdjustments": schema(["id"], { id, contrast: num, saturation: num }),
...
const AGENT_COMMANDS = new Set<CommandId>(["clip.split", "clip.trim", "clip.move", "clip.add", "clip.replaceAll", "clip.moveToTrack", "clip.setEnabled", "clip.setVolume", "clip.setOpacity", "clip.setColorAdjustments", "overlay.addText", "overlay.addImage", "overlay.update", "overlay.delete", "subtitle.edit", "subtitle.delete", "subtitle.retime", "subtitle.clear", "subtitle.replaceAll", "track.addVideo", "track.removeVideo", "track.rename", "track.setLocked", "track.setMuted", "track.setHeight", "track.reorder"]);
