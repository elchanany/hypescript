...
## כמה סוכנים בו-זמנית (חובה — נשרף פעמיים)

בפרויקט הזה רצים לפעמים כמה סוכנים במקביל. שני כשלים אמיתיים כבר קרו:
ארבעה `next build` נלחמו על אותה תיקיית `.next` ונתקעו לשעות, ואז סוכן שניסה
לנקות זיהה בילד **חי** כתקוע והרג אותו.

**חוקים:**

1. **לעולם אל תריץ `npm run build` ישירות.** הרץ `node scripts/agent-build.mjs --name=<נושא>`.
   כל סוכן מקבל `distDir` משלו, ולכן אין התנגשות מלכתחילה.
2. **לפני שהורגים תהליך כלשהו — `node scripts/agent-build.mjs --list`.**
   תהליך שמופיע ברשימה שייך לסוכן אחר שעובד עכשיו. אל תהרוג אותו בלי לשאול
   את המשתמש. הרשימה מנוקה אוטומטית מתהליכים מתים, אז מה שמופיע בה — חי.
3. **`mtime` של קובץ אינו הוכחה שסשן מת.** סוכן יכול לערוך קובץ אחד ולא לגעת
   באחרים במשך שעות. בדוק את הרישום ואת התהליכים בפועל, לא דגימת קבצים.
4. **`git add` תמיד ממוקד לנתיבים שלך.** לעולם לא `git add -A` — הוא יבלע
   עבודה באמצע של סוכן אחר.
5. **לפני commit** — `git status` מלא, ווודא שכל קובץ מוכר לך. קובץ שאינך מזהה
   הוא כנראה של סוכן אחר; השאר אותו.
6. `node scripts/agent-build.mjs --clean` מוחק שאריות `.next-agent-*` ומדלג על
   כאלה שבשימוש. בטוח להרצה בכל עת.
