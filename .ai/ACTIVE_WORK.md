...
## 2026-08-23 — fix B-39 cutting behavior (breath detection, estimator contamination, pacing loop)
...
- **Test coverage added:** `web/lib/audio/breathDetection.test.ts` (new, 28 tests, synthesis-based regression: reverting fix fails 16/28); `web/lib/agent/editPipeline.test.ts` block "הקצב שנקבע בבריף מגיע בפועל לחיתוך" (4 tests: brief→plan, arg overrides brief, loop overrides arg, tight cut ≠ text loss); `web/lib/audio/calibration.test.ts` (invariant recast against `roomFloorDb`); deleted `web/lib/audio/__diag_mp3.test.ts` (temp diagnostic with stale `SILENCE_MARGIN`).
- **Verification:** `npx tsc --noEmit` clean. `npx vitest run` 101 files / 1023 tests all pass.
- **Ancillary fix during verification:** 3 type errors in `web/components/LandingCreativeStack.tsx` (commit 1670dec: provider list 6→4, leaving 3 dead JSX branches touching deleted `provider.mark` and kinds). Removed—dead code only.
...
## 2026-08-22 — user-selected Brain + Play raster master

- Replaced the rejected segmented-lobe draft with the exact user-selected concept: two continuous mirrored hemispheres, four restrained engraved folds, straight inner H rails and an isolated rounded play triangle.
- Regenerated every favicon, PWA icon, horizontal lockup, social card and plan image through the existing deterministic asset pipeline; the 32px derivative and full lockup were visually inspected.
- Verification: brand asset tests 4/4, `tsc --noEmit`, and isolated production build `selected-brain-logo` pass.
