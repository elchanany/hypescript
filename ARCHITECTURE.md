...
## החלטת מפתח: כיול עצמי במקום ספים מוחלטים

כל סף אקוסטי מוחלט נכון להקלטה אחת ושגוי לאחרת — מיקרופון דש, מיקרופון מצלמה
באולם עם הד, וטלפון עם מזגן מייצרים מספרים שונים לחלוטין לאותו צליל. מוצר
שמכויל ידנית על הקלטה אחת נשבר אצל המשתמש הבא.

`audio/calibration.ts` פותר את זה בעזרת תיוג שקיים כבר בחינם: **התמלול**.
חלונות המילים הם מסגרות דיבור ודאיות; אמצע פערים ארוכים הוא רעש הרקע האמיתי.
משתי האוכלוסיות בונים לכל קובץ בנפרד את ההתפלגות שלו, וכל "סף" הופך למיקום
יחסי בין השתיים (`percentileRank`) במקום למספר קבוע. סף הדיבור נגזר כ-
`noise.p90 + 0.3·(speech.p10 − noise.p90)`, והמסווג משווה שטיחות/פסים/התקפה
מול הפרופיל הספקטרלי של הדיבור *באותה הקלטה*.

שלוש שכבות הגנה, לפי סדר:
1. **כיול** — כשההפרדה בין דיבור לרעש ≥6dB, הכל נגזר מהקובץ.
2. **נסיגה מוצהרת** — הקלטה רועשת מדי ⇒ `reliable=false`, ברירות מחדל שמרניות,
   והמשתמש רואה את זה בדוח. אין העמדת פנים של דיוק.
3. **תיקון מבני** — גבול שנכנס לתוך מילה שנשמרת מורחב החוצה תמיד
   (`repairedEdges`). האינווריאנטה "לא חותכים מילה" אינה תלויה בכיול מוצלח.

`lib/audio/calibration.test.ts` מריץ את אותו תוכן בארבעה תנאי הקלטה
(אולפן / אולם / טלפון עם מזגן / הקלטה חלשה) ומאמת שהחיתוך יוצא זהה, שהסף
הנגזר שונה ביניהם ביותר מ-10dB, ושגם עם סף אבסורדי אף מילה אינה נחתכת.
